...
function Board:tileCreate(x_pos, y_pos)
	local possColors = {4, 9, 11, 12, 17, 18}
	local color = possColors[math.floor(math.random(self.colorCount))]
...
function Board:initializeTiles()
	...
	for tileY = 1, 8 do
		...
		for tileX = 1, 8 do
			...
			local tile = self:tileCreate(tileX, tileY)
	...
	for x = 1, 8 do
		for y = 8, 1, -1 do
			...
			if not tile then
				...
				local tile = self:tileCreate(x, y)
				tile.y = -64
				self.tiles[y][x] = tile
